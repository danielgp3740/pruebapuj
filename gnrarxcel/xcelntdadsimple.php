...
->setTitle("Entidades")
->setSubject("Semaforo por Entidades")
->setDescription("Metas de Producto por Entidades.")
->setKeywords("Excel Office 2007 openxml php")
->setCategory("Semaforo Metas de Producto por Entidades");
...
$styleFuenteLetra = array(
  ...
  $reporte_entidad=$principal_sql->entidad($cnxn_pag, $persona_entidad);

  $numero_registro=0;
  while ($data_entidad=$cnxn_pag->obtener_filas($reporte_entidad)){
      $ent_nombre=$data_entidad['ent_nombre'];
      $ent_descripcion=$data_entidad['ent_descripcion'];
      $ent_codigo=$data_entidad['ent_codigo'];

      $hoja_nombre=str_replace (' ', '', $ent_nombre);
      $hoja_nombre=str_replace (',', '', $hoja_nombre);
      $hoja_nombre=substr($hoja_nombre, 0, 20);

      if($numero_registro > 0){
        $objPHPExcel->createSheet();
        $objPHPExcel->setActiveSheetIndex($numero_registro);
        $objPHPExcel->getActiveSheet()->setTitle($hoja_nombre);
      }
      else{
        $objPHPExcel->getActiveSheet()->setTitle($hoja_nombre);
      }
      ...
      */
      ->setCellValue('I1', 'Esperado Cuatrenio')
      ->setCellValue('J1', 'Acumulado')
      ...
      */
      ...
      $reporte_resultadoxproducto=$reportes_sql->reporte_metaresultadoproducto($cnxn_pag, $eac_codigo, $ent_codigo);

      $numero_metapro=1;
      $numero_excel=$numero_metapro+1;

      for($yearv=2016;$yearv<=2017;$yearv++){
            $numerorojo[$yearv]=0;
      ...
      while ($data_metaproductoescenario=$cnxn_pag->obtener_filas($reporte_resultadoxproducto)) {
            ...
            ->setCellValue('G'.$numero_excel, $mpr_lineabase)
            ->setCellValue('H'.$numero_excel, $mpr_valorcuatrenio);
...
///////////////////////////////////////////////////////////////////////////////////////////////
            ...
            $objPHPExcel->setActiveSheetIndex($numero_registro)
            ->setCellValue('I'.$numero_excel, $totalesperadometa)
            ->setCellValue('J'.$numero_excel, $valoracumuladoejecutado)
            ->setCellValue('K'.$numero_excel, $positivosigno.abs($cumplimientovigencias));
            ...
            $objDrawing->setHeight(18);
            $objDrawing->setCoordinates('K'.$numero_excel);
            $objDrawing->setOffsetX(30);
            $objDrawing->setOffsetY(4);
            $objDrawing->setWorksheet($objPHPExcel->getActiveSheet($numero_excel));
    ...
    for($yearv=2016;$yearv<=2017;$yearv++){
      ...
      $objPHPExcel->setActiveSheetIndex($numero_registro)
      ->setCellValue('A'.$numero_finalregistro, "Gris ")
      ...
      $categories = array(new PHPExcel_Chart_DataSeriesValues('String', $hoja_nombre.'!$A$'.$numerograficainicio.':$A$'.$numerograficafinal,NULL,7));

      // definir origem dos valores
      $values = array(new PHPExcel_Chart_DataSeriesValues('Number', $hoja_nombre.'!$B$'.$numerograficainicio.':$B$'.$numerograficafinal, NULL, 7));
      ...
      $title->setCaption($ent_nombre.': Vigencia '.$yearv);
  ...
  header('Content-Disposition: attachment;filename="Semaforo_entidad_'.$fecha_generar.'.xlsx"');
