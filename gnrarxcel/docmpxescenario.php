...
$tabla = "
    <table border='1'>
          ...
          <th>Meta Producto </th>
          <th>Linea Base MP </th>
          <th>Valor Cuatrenio MP </th>
          <th>Vigencia ".$vigencia_genrar." </th>
          <th>Municipio </th>
          <th>Valor Avance </th>
      ...
      $tabla.="<th bgcolor='#E5E5E5'>Total P. Genero</th>";
      ...
      while($data_listampxescenario=$cnxn_pag->obtener_filas($reporte_escenarioadmin)){
        ...
        $tabla.=    "<td>".$mun_nombre."</td>
                ...
                for($numeroff=1;$numeroff<$numero_fuentes;$numeroff++){
                    ...
                    //inicio contratos
                    $contratoconvenio="";
                    $mepro_contrato=$principal_sql->contratoconvenio_listar($cnxn_pag, $vej_codigo, $vigencia);
                    while($data_contrato=$cnxn_pag->obtener_filas($mepro_contrato)){
                        $cej_numero=$data_contrato['cej_numero'];
                        $cej_tipo=$data_contrato['cej_tipo'];
                        if($cej_tipo=='1'){
                          $tipodoc='Contrato';
                        }
                        elseif($cej_tipo=='2'){
                        ...
                        $cej_year=$data_contrato['cej_year'];

                        $contratoconvenio.=$tipodoc." ".$cej_numero." de ".$cej_year.", ";
                    }

                      $tabla.="<td bgcolor='#FFFFFF'>".$contratoconvenio."</td>";
                    //fin contratos
